...
contract YogiToken is ERC20, Ownable {
    using SafeMath for uint256;

    address public admin;
    uint public availableTokensICO;
    
    string public tokenname = "Yogi Bear";
    string public tokensymbol = "YOGI";
    uint8 public tokendecimals = 18;
    uint256 public totalTokenSupply = 3140000000 * 10 ** tokendecimals;
    
    mapping(address => uint256) balances;
    mapping(address => mapping (address => uint256)) allowed;
    
    constructor(uint256 _ICOTokens) ERC20(tokenname, tokensymbol) {
        _ICOTokens = _ICOTokens.mul(10 ** tokendecimals);

        _mint(msg.sender, totalTokenSupply - _ICOTokens);
        _mint(address(this), _ICOTokens);

        availableTokensICO = _ICOTokens;
    ...
    function start(
        ...
        require(_maxPurchase > 0 && _maxPurchase <= availableTokensICO, '_maxPurchase should be > 0 and <= availableTokensICO');
